...
export default function ShoesPage() {
  ...
  const titles = ["Shoes Store", "Baebe Boo Storefront"] as const;
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "products" }, (payload) => {
        ...
        if (payload.eventType === "UPDATE") {
          setItems((prev) => {
            const wasShoes = oldRow?.category === "shoes";
            const isShoes = newRow?.category === "shoes" && newRow?.is_active !== false;

            if (wasShoes && !isShoes) return prev.filter((p) => p.id !== oldRow?.id);

            if (isShoes && newRow?.id) {
        ...
        <div className="max-w-6xl mx-auto px-6">
          {loading ? (
            ...
            <div className="rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
              <div className="text-sm font-semibold">Couldn’t load shoes</div>
